...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [LAYER_0] = LAYOUT_ansi_103(
        KC_MUTE,  KC_F13,   KC_F14,   KC_F15,    CW_TOGG,   KC_BRID,  KC_BRIU,  KC_MCTL,  KC_LPAD,  RGB_VAD,  RGB_VAI,  KC_MPRV,  KC_MPLY,  KC_MNXT,  KC_MUTE,  KC_VOLD,    KC_VOLU,  KC_DEL,             RGB_MOD,
        ...
        KC_P0,              KC_PDOT,             KC_LCTL,  KC_LOPTN, KC_LCMMD,                               KC_SPC,                                 KC_RCMMD, MO(LAYER_1), KC_RCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT),
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {[LAYER_0] = {ENCODER_CCW_CW(KC_VOLD, KC_VOLU)}, [LAYER_1] = {ENCODER_CCW_CW(RGB_VAD, RGB_VAI)}, [LAYER_2] = {ENCODER_CCW_CW(KC_VOLD, KC_VOLU)}, [LAYER_3] = {ENCODER_CCW_CW(RGB_VAD, RGB_VAI)}};
#endif
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    if (host_keyboard_led_state().caps_lock) {
        RGB_MATRIX_INDICATOR_SET_COLOR(LSHIFT_LED_INDEX, 255, 255, 255);
        RGB_MATRIX_INDICATOR_SET_COLOR(RSHIFT_LED_INDEX, 255, 255, 255);
    } else {
        if (!rgb_matrix_get_flags()) {
            RGB_MATRIX_INDICATOR_SET_COLOR(LSHIFT_LED_INDEX, 0, 0, 0);
            RGB_MATRIX_INDICATOR_SET_COLOR(RSHIFT_LED_INDEX, 0, 0, 0);
        }
    }
